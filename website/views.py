...
def lost_objects():
    ...
    user_objects = LostObjects.query.filter(LostObjects.loster.contains(current_user.email)).all()
    lost_objects = LostObjects.query.all()
    return render_template("lost_objects.html", user=current_user, lost_objects=lost_objects, user_objects=user_objects)
...
def search_lost_objects():
    if request.method == 'POST':
        ...
        if query:
            ...
            user_objects = LostObjects.query.filter(LostObjects.loster.contains(current_user.email) & (LostObjects.description.contains(query) | LostObjects.classifier.contains(query))).all()
...
def search_lost_object_location():
    if request.method == 'POST':
        ...
        if query:
            ...
            user_objects = LostObjects.query.filter(LostObjects.loster.contains(current_user.email) & LostObjects.place.contains(query)).all()
...
def search_lost_object_classifier():
    if request.method == 'POST':
        ...
        if query:
            ...
            logger.debug(f'Query: {query}')
            # query the lost objects
            user_objects = LostObjects.query.filter(LostObjects.loster.contains(current_user.email) & LostObjects.classifier.contains(query)).all()
...
# make a /search-lost-object-date route that would take a query string and search for the lost objects by date
...
def search_lost_object_date():
    if request.method == 'POST':
        ...
        # query the lost objects
        user_objects = LostObjects.query.filter(LostObjects.loster.contains(current_user.email) & LostObjects.lost_date.contains(date)).all()
...
# make a route to see all the found objects
@views.route('/found_objects', methods=['GET'])
def found_objects():
    # query all the found objects
    user_objects = FoundObjects.query.filter(FoundObjects.founder.contains(current_user.email)).all()
    found_objects = FoundObjects.query.all()
    return render_template("found_objects.html", user=current_user, found_objects=found_objects, user_objects=user_objects)
...
# make a /search-found-objects route that would take a query string and search for the found objects
...
def search_found_objects():
    if request.method == 'POST':
        ...
        if query:
            # log the query
            logger.debug(f'Query: {query}')
            # query the found objects (filter the found objects by the query: they must not necessarily be exactly the same, but as long as there are some words that are the same in the classifier or description, then it should be returned)
            user_objects = FoundObjects.query.filter(FoundObjects.founder.contains(current_user.email) & (FoundObjects.description.contains(query) | FoundObjects.classifier.contains(query))).all()
...
# make a /search-found-object-location route that would take a query string and search for the found objects by location
...
def search_found_object_location():
    if request.method == 'POST':
        ...
        if query:
            ...
            user_objects = FoundObjects.query.filter(FoundObjects.founder.contains(current_user.email) & FoundObjects.place.contains(query)).all()
...
# make a /search-found-object-classifier route that would take a query string and search for the found objects by classifier
...
def search_found_object_classifier():
    if request.method == 'POST':
        ...
        if query:
            ...
            logger.debug(f'Query: {query}')
            # query the found objects
            user_objects = FoundObjects.query.filter(FoundObjects.founder.contains(current_user.email) & FoundObjects.classifier.contains(query)).all()
...
# make a /search-found-object-date route that would take a query string and search for the found objects by date
@views.route('/search-found-object-date', methods=['GET', 'POST'])
def search_found_object_date():
    if request.method == 'POST':
        date = request.form.get('found_date')
        # log the query
        logger.debug(f'Date: {date}')
        # query the found objects
        user_objects = FoundObjects.query.filter(FoundObjects.founder.contains(current_user.email) & FoundObjects.found_date.contains(date)).all()
